...
class TicketManager(models.Manager):
    ...
    def send_ticket_mail(self, ticketid):
        ticketmail = self.raw(
            """SELECT ticket.id, ticket.ticketlog,  ticket.comments, ticket.ticketdesc, ticket.cdtz,ticket.status, ticket.ticketno, ticket.level, bt.buname,
                ...
                WHERE ticket.id = ANY(%s)""",
            [ticketid if isinstance(ticketid, (list, tuple)) else [ticketid]],
        )
    ...
    def get_ticketlist_for_escalation(self):
        ...
        return (
            utils.runrawsql(raw_queries.get_query("get_ticketlist_for_escalation"))
            or self.none()
        )
