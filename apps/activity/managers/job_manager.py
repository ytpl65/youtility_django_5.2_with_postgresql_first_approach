...
class JobManager(models.Manager):
    ...
    def getgeofence(self, peopleid, siteid):
        qset = (
            self.filter(people_id=peopleid, bu_id=siteid, identifier="GEOFENCE")
            .select_related(
                "geofence",
            )
            .annotate(geofencejson=AsGeoJSON("geofence"))
            .values(
                "geofence__id",
                "geofence__gfcode",
                "people_id",
    ...
    def get_checkpoints_for_externaltour(self, job):
        qset = (
            self.select_related("identifier", "butype", "parent")
            .annotate(bu__buname=F("buname"))
            .filter(parent_id=job.bu_id)
            .values(
                "buname",
                "id",
                "bucode",
    ...
    def get_listview_objs_schdexttour(self, request):
        ...
        qset = (
            ...
            .select_related("pgroup", "sgroup", "people")
            .values(
                "assignedto",
                "sitegrpname",
                "israndomized",
    ...
    def get_sitecheckpoints_exttour(self, job, child_jobid=None):
        ...
        if child_jobid:
            return (
                qset.filter(jobid=child_jobid).values(*fields).order_by("seqno")
                or self.none()
            )
        return qset or self.none()
    ...
    def handle_geofencepostdata(self, request):
        ...
        if R["action"] == "create" or R["action"] == "edit":
            ...
            PostData = {
                "jobname": f"{R['gfcode']}-{R['people__peoplename']}",
                "identifier": "GEOFENCE",
            ...
            fromdate = datetime.strptime(
                R[f"data[{pk}][fromdate]"], "%Y-%m-%dT%H:%M:%SZ"
            ).date()
            uptodate = datetime.strptime(
                R[f"data[{pk}][uptodate]"], "%Y-%m-%dT%H:%M:%SZ"
            ).date()
            starttime = datetime.strptime(
                R[f"data[{pk}][starttime]"], "%H:%M:%S"
            ).time()
            ...
            PostData = {
                "jobname": f"{R['gfcode']}-{R[f'data[{pk}][people__peoplename]']}",
                "identifier": "GEOFENCE",
        ...
        qset = (
            self.annotate(
                assignedto=Case(
                    ...
                    When(
                        people_id=1, then=Concat(F("pgroup__groupname"), V(" [GROUP]"))
                    ),
                )
            )
            .filter(
    ...
    def handle_save_checkpoint_guardtour(self, request):
        ...
        try:
            if R["action"] == "create":
                if self.filter(
                    qset_id=checkpoint["qsetid"],
                    asset_id=checkpoint["assetid"],
                ...
                if updated := self.filter(pk=R["pk"]).update(
                    **child_job, muser=request.user, mdtz=mdtz
                ):
                    ID = R["pk"]
            ...
            if "expirytime_gte_0_ck" in str(e):
                return {
                    "data": [],
                    "error": "Invalid Expiry Time. It must be greater than or equal to 0.",
                }
    ...
    def handle_save_checkpoint_sitetour(self, request):
        ...
        try:
            ...
            if R["action"] == "edit":
                ...
                if updated := self.filter(id=R["jobid"]).update(
                    **child_job_post_data, muser=request.user, mdtz=mdtz
                ):
                    ID = R["jobid"]
...
class JobneedManager(models.Manager):
    ...
    def get_schedule_for_adhoc(self, pdt, peopleid, assetid, qsetid, buid):
        return self.raw(
            "select * FROM get_schedule_for_adhoc(%s, %s, %s, %s, %s)",
            params=[pdt, buid, peopleid, assetid, qsetid],
        )

    def get_jobneedmodifiedafter(self, mdtz, peopleid, siteid):
        mdtzinput = (
            mdtz
            if isinstance(mdtz, datetime)
            else datetime.strptime(mdtz, "%Y-%m-%d %H:%M:%S")
        )
        return (
            self.raw(
                "select * from fn_getjobneedmodifiedafter(%s, %s, %s) as id",
    ...
    def get_hdata_for_report(self, pk):
        qset = self.raw(
            """WITH RECURSIVE nodes_cte(jobneedid, parent_id, jobdesc, people_id, qset_id, plandatetime, cdtz, depth, path, top_parent_id, pseqno, buid) AS
        (
        ...
        WHERE jobneed.parent_id <> -1  ORDER BY pseqno asc, jobdesc asc, pseqno, cseqno asc""",
            [pk],
        )
        return qset or self.none()
    ...
    def get_adhoctasks_listview(self, R, task=True):
        ...
        qset = (
            self.select_related("performedby", "qset", "asset")
            .filter(
                identifier__in=idf,
                jobtype="ADHOC",
    ...
    def get_task_list_jobneed(self, related, fields, request, id=None):
        ...
        qobjs = (
            ...
            .exclude(parent__jobdesc="NONE", jobdesc="NONE")
            .values(*fields)
    ...
    def get_internaltourlist_jobneed(self, request, related, fields):
        ...
        qobjs = (
            self.annotate(
                **assignedto,
                client_name=F("client__buname"),
                site_name=F("bu__buname"),
                no_of_checkpoints=Count("jobneed", distinct=True),
                completed=Count(
                    "jobneed", filter=Q(jobneed__jobstatus="COMPLETED"), distinct=True
                ),
                missed=Count(
                    "jobneed",
                    filter=Q(jobneed__jobstatus__in=["ASSIGNED", "AUTOCLOSED"]),
                    distinct=True,
                ),
            )
            .select_related(*related)
    ...
    def get_externaltourlist_jobneed(self, request, related, fields):
        ...
        qset = (
            self.annotate(**assignedto)
            .select_related(*related)
            .filter(
                Q(bu_id__in=S["assignedsites"])
                | Q(sgroup_id__in=S["assignedsitegroups"]),
                parent_id=1,
                plandatetime__date__gte=P["from"],
                plandatetime__date__lte=P["to"],
                jobtype="SCHEDULE",
        ...
        if P.get("alerts") and P.get("alerts") == "ROUTEPLAN":
            ...
            alert_qset_parents = list(
                set(alert_qset.values_list("parent_id", flat=True))
            )
            qset = (
                self.filter(id__in=alert_qset_parents)
                .annotate(**assignedto)
                .values(*fields)
            )
        return qset or self.none()
    ...
    def get_tourdetails(self, R):
        qset = (
            ...
            .select_related("parent", "asset", "qset")
            .filter(parent_id=R["parent_id"])
            .values(
                "asset__assetname",
                "asset__id",
                "qset__id",
    ...
    def get_ppm_listview(self, request, fields, related):
        ...
        qobjs = (
            self.select_related("people", "bu", "pgroup", "client")
            .annotate(
                assignedto=Case(
                    When(
                        pgroup_id=1,
    ...
    def get_taskchart_data(self, request):
        ...
        total_sch = (
            ...
            .aggregate(
                assigned=Count(
                    Case(
                        When(jobstatus="ASSIGNED", then=1), output_field=IntegerField()
                    )
                ),
                completed=Count(
                    Case(
                        When(jobstatus="COMPLETED", then=1), output_field=IntegerField()
                    )
                ),
                autoclosed=Count(
                    Case(
                        When(jobstatus="AUTOCLOSED", then=1),
    ...
    def get_tourchart_data(self, request):
        ...
        total_schd = (
            ...
            .aggregate(
                ...
                partially_completed=Count(
                    Case(
                        When(jobstatus="PARTIALLYCOMPLETED", then=1),
                        output_field=IntegerField(),
                    )
                ),
                total=Count("id"),
    ...
    def get_alertchart_data(self, request):
        ...
        aggreated_data = qset.aggregate(
            task_alerts=Count(
                "id", filter=Q(Q(parent_id__in=[1, -1, None]), identifier="TASK")
            ),
    ...
    def get_expired_jobs(self, id=None):
        ...
        if not id:
            qset = (
                self.select_related(*related_fields)
                .annotate(**annotation)
                .filter(
                    ~Q(id=1),
                    ...
                    ~Q(
                        ...
                        & (
                            Q(other_info__email_sent=True)
                            | Q(other_info__ticket_generated=True)
                        )
                    ),
                    Q(parent_id=1),
                    Q(
    ...
    def get_ppmchart_data(self, request):
        ...
        total_schd = (
            ...
            .aggregate(
                completed=Count(
                    Case(When(jobstatus="COMPLETED"), then=1),
                    output_field=IntegerField(),
                ),
                assigned=Count(
                    Case(When(jobstatus="ASSIGNED"), then=1),
                    output_field=IntegerField(),
                ),
                autoclosed=Count(
                    Case(When(jobstatus="AUTOCLOSED"), then=1),
                    output_field=IntegerField(),
                ),
    ...
    def get_schedule_for_adhoc(
        self, qsetid, peopleid, assetid, buid, starttime, endtime
    ):
        qset = (
            self.filter(
                ~Q(jobtype="ADHOC"),
    ...
    def get_task_summary(self, request, params):
        ...
        while current_date <= uptodate:
            ...
            tot_completed = qset.filter(
                jobtype="SCHEDULE", jobstatus="COMPLETED"
            ).count()
            ...
            record = {
                ...
                "closed_jobs": qset.filter(
                    jobtype="SCHEDULE", jobstatus="AUTOCLOSED"
                ).count(),
                "closed_jobs": qset.filter(
                    jobtype="SCHEDULE", jobstatus="ASSIGNED"
                ).count(),
    ...
    def get_latlng_of_checkpoints(self, jobneed_id):
        ...
        qset = (
            self.filter(parent_id=jobneed_id)
            .annotate(gps=AsGeoJSON("gpslocation"))
            .values(
                "gps",
                "seqno",
                "starttime",
        ...
        for q in qset:
            ...
            checkpoints.append(
                [[gps["coordinates"][1], gps["coordinates"][0]], q["seqno"]]
            )
            info.append(
                {
                    "starttime": self.formatted_datetime(
                        q["starttime"], q["ctzoffset"]
                    ),
    ...
    def get_path_of_checkpoints(self, jobneed_id):
        site_tour_parent = (
            self.annotate(path=AsGeoJSON("journeypath")).filter(id=jobneed_id).first()
        )
        if (
            site_tour_parent.jobstatus
            in (self.model.JobStatus.COMPLETED, self.model.JobStatus.PARTIALLYCOMPLETED)
            and site_tour_parent.path
        ):
            ...
            between_latlngs = Tracking.objects.filter(
                reference=site_tour_parent.uuid
    ...
    def get_latest_location_of_rider(self, jobneed_id):
        ...
        devl = (
            DeviceEventlog.objects.filter(people_id=people.id)
            .order_by("-receivedon")
            .first()
        )
        trac = (
            Tracking.objects.filter(people_id=people.id)
            .order_by("-receiveddate")
            .first()
        )
        ...
        return {
            ...
            "mobno": people.mobno,
            "email": people.email,
            "time": self.formatted_datetime(
                getattr(event, time_key), site_tour.ctzoffset
            ),
    ...
    def get_job_needs(self, people_id, bu_id, client_id):
        ...
        group_ids = (
            pm.Pgbelonging.objects.filter(people_id=people_id)
            .exclude(pgroup_id=-1)
        ...
        job_needs_filter = Q(bu_id=bu_id) & Q(client_id=client_id) & ~Q(
            ...
            | (
                Q(plandatetime__lte=datetime.now())
                & Q(expirydatetime__gte=datetime.now())
            )
        ) | (
            Q(other_info__isdynamic=True) & Q(mdtz__date__range=[today, tomorrow])
        ) & Q(
            client_id=client_id
        ) & Q(
            bu_id=bu_id
        ...
        job_needs = (
            self.annotate(
                ...
                isdynamic=F("other_info__isdynamic"),
            )
            .filter(job_needs_filter)
            .values(*fields)
        )
        return job_needs
    ...
    def get_external_tour_job_needs(self, people_id, bu_id, client_id):
        ...
        parentqset = self.filter(
            (
                Q(plandatetime__date__range=[today, tomorrow])
                | (Q(plandatetime__gte=now()) & Q(expirydatetime__lte=now()))
            )
            & (
                Q(people_id=people_id)
                | Q(cuser_id=people_id)
...
class JobneedDetailsManager(models.Manager):
    ...
    def update_ans_muser(self, answer, peopleid, mdtz, jnid):
        ...
        return self.filter(jobneed_id=jnid).update(
            muser_id=peopleid, answer=answer, mdtz=_mdtz
        )

    def get_jnd_observation(self, id):
        qset = (
            self.select_related("jobneed", "question")
            .filter(jobneed_id=id)
            .order_by("seqno")
        )
        return qset or self.none()
    ...
    def get_jndofjobneed(self, R):
        qset = (
            self.filter(jobneed_id=R["jobneedid"])
    ...
    def get_e_tour_checklist_details(self, jobneedid):
        qset = (
            self.filter(jobneed_id=jobneedid)
            ...
            series.append(
                {
                    ...
                    "data": list(
                        qset.filter(jobneed__asset_id=asset_id).values_list(
                            "starttime", "answer_as_float"
                        )
                    ),
                }
            )
            ...
            series.append(
                {
                    ...
                    "data": list(
                        qset.filter(question_id=question_id).values_list(
                            "starttime", "answer_as_float"
                        )
                    ),
                }
